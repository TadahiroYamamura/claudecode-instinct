...
instinct-cli（Dolt に読み書きする Go CLI ツール）が Dolt にアクセスするための手段を選定する必要があった。
...
- `instinct-cli` バイナリは初回実行時に自動ビルドし、`.gitignore` で管理する（プリコンパイル済みバイナリはコミットしない）
...
- Python / Node.js から Dolt を直接使う embedded driver は存在しないため、Dolt アクセスは instinct-cli（Go 製）に集約する
